...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_notie__ = __webpack_require__(7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_notie___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_1_notie__);
...
$(document).ready(function () {
	...
	window.notie = __WEBPACK_IMPORTED_MODULE_1_notie___default.a;
	...
	window.addToCart = function addToCart(domId, encrypted) {
		var qty = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1;

		var $btn = $('*[data-dom="' + domId + '"]');
		...
		$.ajax({
			...
			success: function success(data) {
				...
				__WEBPACK_IMPORTED_MODULE_0_sweetalert___default()('Success', 'Item Successfully Added To Your Cart', 'success').then(function (success) {
			...
			error: function error(err) {
				return __WEBPACK_IMPORTED_MODULE_0_sweetalert___default()('Error', 'Failed to add item to cart!', 'error');
			}
		});
	};
	...
	$("#form-address").submit(function () {
		...
		$.ajax({
			...
			error: function error() {
				return __WEBPACK_IMPORTED_MODULE_0_sweetalert___default()('Oops!', 'Looks like something went wrong!', 'error');
			},
			success: function success(data) {
				if (data.success == true) {
					$("#address_form").modal('hide');
					__WEBPACK_IMPORTED_MODULE_0_sweetalert___default()("New address has been added", "A new address has been added to your address list.", "success").then(function (value) {
						window.location.reload();
					});
				} else if (data.success == false) {
					$("#address_form").modal('hide');
					__WEBPACK_IMPORTED_MODULE_0_sweetalert___default()(data.title, data.data, "warning");
				}
			}
		});
		return false;
	...
	var loadData = function loadData($this, type, id) {
		var endpoint = void 0,
		    text = void 0;
		if (type == 'province') {
		...
		$.ajax({
			...
			success: function success(xhr, status, data) {
				if (data.status == 200) {
					var options = "<option disabled selected>Pilih " + text + "</option>";
		...
		$this.on("click", function () {
			__WEBPACK_IMPORTED_MODULE_0_sweetalert___default()({
				...
				if (willDelete) {
					...
					__WEBPACK_IMPORTED_MODULE_0_sweetalert___default()('Safe!', 'Your address are safe!', 'success');
				}
			});
	...
	window.appendQuery = function (key, value) {
	...
	window.reformat = function () {
		var cook = __WEBPACK_IMPORTED_MODULE_2_js_cookie___default.a;
...
$(document).ready(function () {
	...
	$('#filterBy').on('changed.bs.select', function (e) {
		var $choosed = $(this).children(':selected').attr('value');
		var url = '';
		console.log($choosed);
	...
	$('div.filter-category-wrapper :checkbox').on('change', function (e) {
		...
		if (final_query == '') {
			...
			$.each(parseQuery(), function (i, d) {
				var temp_query = [];
				if (i != 'category') {
					var url = appendQuery(i, d);
					var new_url = removeQuery(url, 'category');
					window.history.pushState({ "html": "g", "pageTitle": "l" }, "", new_url);
				}
			});
			return __do_category_ajax();
		} else {
			var url = appendQuery('category', final_query);
			window.history.pushState({ "html": "g", "pageTitle": "l" }, "", url);
		...
		function __do_category_ajax() {
			var param = '_token=' + $("meta[name=csrf-token]").attr('content') + '&';
			var pathArray = window.location.pathname.split('/');
			var urlSegment = pathArray[1];
