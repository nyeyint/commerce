                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th>
                                            ...
                                            @if($row->display_name == 'Backend Name')
                                                Name
                                            @else
                                                {{ $row->display_name }}
                                            @endif
                                            ...
                                            <td>
                                                ...
                                                @elseif($row->type == 'checkbox')
                                                    @if($options && property_exists($options, 'on') && property_exists($options, 'off'))
                                                        @if($row->field == 'status') 
                                                            @if($data->{$row->field} == 1)
                                                                   <input id="status-{{$data->id}}" data-prefix="{{explode('.', $data->prefix)[0]}}"  name="status" data-name="Status" class="toggleswitch statusCheck" data-on="Enabled" checked="checked" data-off="Disabled" type="checkbox">
                                                            @else
                                                                   <input id="status-{{$data->id}}" data-prefix="{{explode('.', $data->prefix)[0]}}" name="status" data-name="Status" class="toggleswitch statusCheck" data-on="Enabled" data-off="Disabled" type="checkbox">
                                                            @endif
                                                        @else
                                                            @if($data->{$row->field})
    ...
    <script>
        $(document).ready(function () {
            ...
            @foreach($dataTypeContent as $row)
                    var ajax = true;
                    $('#status-' + {{$row->id}}).change({ajax: ajax} , function(event) {
                        
                        $checked = $(this).is(':checked');
                        if($checked) {
                
                            // ajax for enable payment gateway
                            $.ajax({
                                url: '{{route('payment.enablePaymentGateway', $row->id)}}',
                                type: 'POST',
                                data: '_token={{csrf_token()}}',
                                success: function(data) {
                                    if(data.disabled) {
                                        $els = $('*[data-prefix="' + data.disabled + '"]');
                                    ...
                                    toastr.success('', 'Payment Gateway Enabled Successfully!');
                                },
                                error: function(err) {
                                    toastr.error('', 'Unable to Enable Payment Gateway!');
                                }
                            });
                        }
                        else
                        {
                            // ajax for disable payment gateway
                            if(ajax) {
                                $.ajax({
                                    url: '{{route('payment.disablePaymentGateway', $row->id)}}',
                                    type: 'POST',
                                    data: '_token={{csrf_token()}}',
                                    success: function(data) {
                                        toastr.success('', 'Payment Gateway Disabled Successfully!');
                                    },
                                    error: function(err) {
                                        toastr.error('', 'Unable to Disabled Payment Gateway!');
                                    }
                                });
                            }
                        }
