                                            ...
                                            <td>
                                                ...
                                                @elseif($row->type == 'checkbox')
                                                    ...
                                                    @if($row->field == 'is_active')
                                                        @if($data->{$row->field} == 1)
                                                               <input id="status-{{$data->id}}" data-prefix=""  name="status" data-name="Status" class="toggleswitch statusCheck" data-on="Enabled" checked="checked" data-off="Disabled" type="checkbox">
                                                        @else
                                                               <input id="status-{{$data->id}}" data-prefix="" name="status" data-name="Status" class="toggleswitch statusCheck" data-on="Enabled" data-off="Disabled" type="checkbox">
                                                        @endif
                                                    @else
                                                        {{ $data->{$row->field} }}
                                                    @endif
    ...
    <script>
        $(document).ready(function () {
            ...
            @foreach($dataTypeContent as $row)
                    var ajax = true;
                    $('#status-' + {{$row->id}}).change(function(event) {
                        ...
                        if($checked) {
                            ...
                            $.ajax({
                                url: '{{route('courier.enable', $row->id)}}',
                                type: 'POST',
                                data: '_token={{csrf_token()}}',
                                success: function(data) {
                                    toastr.success('', 'Courier Enabled Successfully!');
                                },
                                error: function(err) {
                                    toastr.error('', 'Unable to Enable Courier!');
                                }
                            });
                        }
                        else
                        {
                            ...
                            $.ajax({
                                url: '{{route('courier.disable', $row->id)}}',
                                type: 'POST',
                                data: '_token={{csrf_token()}}',
                                success: function(data) {
                                    toastr.success('', 'Courier Disabled Successfully!');
                                },
                                error: function(err) {
                                    toastr.error('', 'Unable to Disabled Courier!');
                                }
                            });
                        }
