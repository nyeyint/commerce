                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th>
                                            ...
                                            @endif
                                            @if($row->display_name == 'Short Description')
                                                Description
                                            @elseif($row->display_name == 'Discount This Product?') 
                                                Discount
                                            @elseif($row->display_name == 'Product Code')
                                                Code
                                            @else
                                                {{$row->display_name}}
                                            @endif

                                            {{-- {{ ($row->display_name == 'Short Description') ? 'Description' :  }} --}}
                                            @if ($isServerSide)
                                            ...
                                            <td>
                                                ...
                                                @elseif($row->type == 'select_multiple')
                                                    @if(property_exists($options, 'relationship'))
                                                        ...
                                                        @php 
                                                            $categories = explode(',', preg_replace("/[^a-zA-Z0-9\,]/",'', $data->{$row->field}));
                                                        @endphp
                                                        @foreach($categories as $item)
                                                            <label for="" class="label label-danger">{{getCategoryById($item)->name}}</label>
                                                        @endforeach
                                                ...
                                                @elseif($row->type == 'select_dropdown' && property_exists($options, 'options'))
                                                    ...
                                                    {{getCategoryById($data->{$row->field})->name}}


                                                @elseif($row->type == 'select_dropdown' && $data->{$row->field . '_page_slug'})
                                                    <a href="{{ $data->{$row->field . '_page_slug'} }}">{{ $data->{$row->field} }}</a>
                                                @elseif($row->type == 'date')
                                                {{ $options && property_exists($options, 'format') ? \Carbon\Carbon::parse($data->{$row->field})->formatLocalized($options->format)->diffForHumans() : $data->{$row->field}->diffForHumans() }}
