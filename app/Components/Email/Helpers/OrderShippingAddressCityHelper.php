...
class OrderShippingAddressCityHelper {
	...
	protected function parse() {
      ...
      $address_data = json_decode($this->data['order']->shipping_address);
			$this->parsed = is_array($this->helper)
													?  str_replace($this->helper['helper'], $address_data->city_label, $string)
													:  str_replace($this->helper, $address_data->city_label, $string);
