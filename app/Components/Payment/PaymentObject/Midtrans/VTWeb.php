...
class VTWeb extends BasePaymentGateway {
	 ...
	 */
	 ...
	 public function notify($request) {
		\Log::info('Midtrans notification', $request);
		$orderId = array_key_exists('order_id', $request)
					? $request['order_id']
					: null;

		// before we update the order status in database, we need to make sure
		// the request is come from midtrans server, to check it, then we need to make API Call
		// to midtrans API.
		try	{
		...
		if(array_key_exists($code, $status)) {
			// update order status.
			\Log::info('Update order status to ' . $status[$code]);
